...
### TimeEntryForm 开始时间自动设置（核心机制）
Chrono 的使用理念是"连续记录生活流水账"，所以"下一条 startTime 自动衔接上一条 endTime"是承重墙——8 条不同的触发路径必须给出正确、可预测、与时间轴显示一致的值。两个核心查询函数（`entryStore.getLastEntryEndTimeForDate` 按当天 overlap + clip 到 dayEnd；`getLastEndTimeBeforeOrAt` 按时间轴前驱）；跨午夜停止/手动保存会切换 `selectedDate` 到 endTime 所在日；锚点机制（`lastEndAnchorRef` + 5s 容差）区分"自动衔接"和"用户手动覆盖"。已知遗留：fallback 三处不一致、5s 容差、过去空白日未拦截。**修改前必读** `docs/architecture.md` 中 "TimeEntryForm 开始时间自动设置" 一节。
...
Streaming model: every call uses `streamText` (SDK v5). `streamingEngine.ts` is the pure event translator over `result.fullStream`, mapping SDK events to six UI phase rows: `preparing` / `requesting` / `reasoning` / `composingTool` / `toolCall` / `answering`. `toolCallEngine.ts` is the AI Assistant entry — it builds the system prompt, organizes history, wires `actionRegistry.toSdkTools({onConfirmRequired})`, then delegates to `streamingEngine`. Each row carries structured debugInfo (MODEL REQUEST, REASONING per-step text streamed live, TOOL INPUT, TOOL CALL input+output); the active `reasoning` row force-expands so reasoning text streams visibly. The SDK handles cross-chunk `tool_call` argument accumulation, multi-step tool loops (`stopWhen: stepCountIs(5)`), and reasoning/`<think>` normalisation. **QuickCapture's `parseTranscript` reuses `streamingEngine` directly with `maxSteps: 1` and an inline `add_entry` shim tool** so the model's proposed entries get captured for user review without writing to the DB. See `docs/ai-assistant.md`.
