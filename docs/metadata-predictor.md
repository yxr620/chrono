...
当路径 A 没有 high-confidence 结果时，还会用输入活动和页面目标名做直接片段匹配。唯一且完整覆盖目标有效片段的 high-confidence 直接命中会优先于路径 A 的 medium 结果；只共享部分片段（例如“看论文”与“论文总结”）不会越过历史护栏。如果直接匹配也只有 medium，则保留路径 A 的结果。
