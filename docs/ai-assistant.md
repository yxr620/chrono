...
这条路径的关键点是：第一次 `thinking` 不是最终回答，而是让模型判断是否要调用工具；`toolCall` 把本地查询结果追加成 `role: tool`；第二次 `thinking` 才基于真实数据组织最终回答。写入型请求会在 `toolCall` 中插入确认卡片，确认细节放在 [Action Registry 文档：确认流程](action-registry.md#确认流程)。
